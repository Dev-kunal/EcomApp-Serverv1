...
try{
  const cartItems=await CartItem.find({userId:userId}).populate({path:'productId',model:'Product'})
  if(!cartItems){
...
try{
  ...
  const productToUpdate=await CartItem.findOneAndUpdate(obj, { $inc: { quantity: updateTerm }},{new: true } )
